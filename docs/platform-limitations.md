...
Workspace screen-sleep and session notifications do not prove authentication has completed. Version 0.1.0 therefore asks for a menu interaction after return. The unreleased automatic-return implementation additionally observes the **undocumented** `CGSSessionScreenIsLocked` field returned through the public `CGSessionCopyCurrentDictionary()` API. It requires an observed lock/unlock cycle, an available desktop, a ready device signal, and a settling delay. This is a compatibility hint with a manual fallback, not a supported authentication-completed API. See [automatic return](automatic-return.md). It uses no undocumented `CGSession` executable, private screen-lock function, or Apple Watch unlock implementation.

...
| Optional BLE scan | Bluetooth permission, requested only when the user enables scanning |
...
There is no app backend, remote command listener, cloud API key, camera, microphone, location tracking, or root helper. The build has no third-party package dependencies. Bluetooth observation does not pair or connect to peripherals. Device identifiers/names are untrusted and are never executed.

## Failure modes and controls

- **False departure:** RSSI can fluctuate, an iPhone can stop advertising, or the user may read without input. The activity veto, smoothing, grace period, pause control, and adjustable idle timeout reduce inconvenience; they do not eliminate it.
- **False presence:** A device may be left on the desk or spoofed. The idle timeout still applies even when a beacon is near. Bluetooth never grants access or unlocks the Mac.

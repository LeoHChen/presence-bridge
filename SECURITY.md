...
| Persistence | Idle threshold plus the selected peripheral's local UUID and sanitized friendly name in local app preferences |

Other discovered names, signal strength, calibration, and state are held in memory. The app caps the named-device list at 50 and expires old discoveries. The selected UUID and friendly name are potentially identifying; do not publish them in bug reports. A bounded in-app activity history is session-only, user-clearable, and intentionally excludes UUIDs, nearby names, and raw RSSI. The app writes no telemetry. Shortcut receipts are written briefly to a private temporary directory. A crash can leave receipts until system cleanup. Apple services may handle Focus synchronization and their own logs.
...
- **Lock-state compatibility:** The v0.2 automatic-return feature reads the undocumented `CGSSessionScreenIsLocked` entry through a public session query. It requires a lock/unlock cycle plus an available desktop and fresh device readiness before resuming an already-active session. The signal is not an authentication API; missing/changed OS behavior can prevent detection, and **I'm back** remains the fallback. Waking or returning nearby alone cannot auto-resume. Pause and restart remain inactive. See [automatic return](docs/automatic-return.md).
...
The script produces an ad-hoc signed local bundle unless `PRESENCE_SIGN_IDENTITY` selects a developer identity. Ad-hoc signing is not notarization or a trust endorsement. Version 0.1.0 is a public experimental preview with this limitation disclosed on the download page and in the archive. Version 0.2.0 can be Developer ID signed; notarization, a wider support matrix, and longer field testing remain release work. Never work around installation issues by disabling SIP or Gatekeeper. Checksums detect mismatched downloads; they do not replace a trusted developer signature.
